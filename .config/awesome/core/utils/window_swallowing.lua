...
local helper = require("core.libs.client")
...
local parent_filter_list = beautiful.parent_filter_list
	...
	or { "firefox", "Gimp", "Google-chrome", "krita" }
local child_filter_list = beautiful.child_filter_list or beautiful.dont_swallow_classname_list or {}
		...
		then
			c:connect_signal("unmanage", function()
				if parent_client then
					helper.turn_on(parent_client)
					helper.sync(parent_client, c)
				end
			end)

			helper.sync(c, parent_client)
			helper.turn_off(parent_client)
		end
	end)
end
